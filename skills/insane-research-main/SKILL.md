...
#### Claim/Source 각주 변환 (옵션)

본문의 `[clm_001] [src_001]` 태그를 표준 마크다운 각주(`[^1]`) 및 `## 각주 (Footnotes)` 섹션으로 자동 치환하려면 다음 스크립트를 실행한다:

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/insane-research-main/scripts/convert_footnotes.py" --session "RESEARCH/{topic}_{timestamp}" --report "RESEARCH/{topic}_{timestamp}/outputs/report.md"
```
...
| `convert_footnotes.py` | **각주 치환기 (선택).** 본문의 claim/source 태그를 마크다운 각주([^1])로 자동 변환 | helper (각주 치환) |
